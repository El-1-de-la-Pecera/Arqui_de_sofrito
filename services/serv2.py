import socket
import utils
from utils import insert_producto
...
print(status)
if (status == 'OK'):
    print('Servicio registro_producto iniciado de forma correcta\n')
    while True:
        received_message = sock.recv(4096).decode('UTF-8')
        print(received_message)
        client_id = received_message[5:10]
        data = eval(received_message[10:])
        ans = insert_producto(data['nombre'], data['precio'], data['stock'], data['categoria'])
